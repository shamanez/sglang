...
<table style={{width: "100%", borderCollapse: "collapse", tableLayout: "fixed"}}>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.05)"}}>Quantize the pipeline-parallel inter-stage activation wire format to 8-bit with per-token scales: <code>int8</code> or <code>fp8</code>. Lossy on the wire; the model always sees the model dtype. Research knob.</td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.02)"}}><code>unset</code></td>
    </tr>
    <tr>
