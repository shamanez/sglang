...
<table style={{width: "100%", borderCollapse: "collapse", tableLayout: "fixed"}}>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.05)"}}>Quantize the pipeline-parallel inter-stage activation wire format with per-token scales: <code>int8</code>, <code>fp8</code> (CUDA/MUSA only), or <code>int4</code>. Lossy on the wire; the model always sees the model dtype. Ignored when <code>pp_size == 1</code>. Research knob.</td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.02)"}}><code>unset</code></td>
    </tr>
    <tr>
